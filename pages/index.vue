<template>
  ...
  >
    ...
    <template v-if="isMounted">
      <div class="coordinates-x d-none d-sm-flex" :style="parallaxStyle">
        <span v-for="n in 15" :key="'x' + n">{{ (n - 1) * 100 }}</span>
      </div>
      <div class="coordinates-y d-none d-sm-flex" :style="parallaxStyle">
...
const dynamicLogoSize = computed(() => {
  if (mobile.value) return 200; // Smaller for phone
